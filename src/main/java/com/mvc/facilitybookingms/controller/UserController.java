...
public class UserController {
    ...
    @ApiResponses({
        ...
        @ApiResponse(responseCode = "400", description = "Invalid request body or email already exists", content = @Content)
    })
    ...
    @Operation(summary = "Get all users", description = "Admin only. Retrieves a list of all users in the system.")
    @ApiResponses({
    ...
    @Operation(summary = "Get a user by ID", description = "Admin can fetch any user. Users can only fetch their own profile.")
    @ApiResponses({
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN') or @userSecurity.isSelf(#id, authentication.name)")
    @Operation(summary = "Update a user", description = "Admin can update any user. Users can only update their own profile.")
    @ApiResponses({
    ...
    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Delete a user", description = "Admin only. Deletes a user by their ID.")
    @ApiResponses({
